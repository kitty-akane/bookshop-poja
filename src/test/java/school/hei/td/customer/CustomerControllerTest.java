...
class CustomerControllerTest {
  ...
  void getById_when_customerExists_shouldReturn200() throws Exception {
    ...
    mockMvc
        .perform(get("/customers/{id}", EXISTING_ID))
  ...
  void getById_when_customerDoesNotExist_shouldThrow404() throws Exception {
    ...
    mockMvc.perform(get("/customers/{id}", NON_EXISTING_ID)).andExpect(status().isNotFound());
  }

  @Test
  void getById_withInvalidId_shouldThrow400() throws Exception {
    mockMvc.perform(get("/customers/{id}", "not-a-number")).andExpect(status().isBadRequest());
  }
  ...
  void update_withNonExistingId_shouldThrow404() throws Exception {
    ...
    mockMvc
        .perform(
            put("/customers/{id}", NON_EXISTING_ID)
                .contentType(MediaType.APPLICATION_JSON)
  ...
  void delete_withExistingId_shouldReturn200() throws Exception {
    ...
    mockMvc.perform(delete("/customers/{id}", EXISTING_ID)).andExpect(status().isOk());
  }
  ...
  void delete_withNonExistingId_shouldThrow404() throws Exception {
    ...
    mockMvc.perform(delete("/customers/{id}", NON_EXISTING_ID)).andExpect(status().isNotFound());
  }
}
